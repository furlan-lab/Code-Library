...
VlnPlot(t, features =  c("nCount_RNA", "nCount_ATAC", "TSS.enrichment", "nucleosome_signal", "TSS.percentile", "Frip", "percent.mt"), group.by = "dataset", pt.size = 0)
...
cd4<-RunUMAP(cd4, dims = 1:12, verbose = T, reduction.name = "SCT_UMAP")
...
cd4<- RunSVD(cd4)
cd4<- RunUMAP(cd4, reduction = 'lsi', dims = 1:12, reduction.name = "ATAC_UMAP")
ElbowPlot(cd4, ndims = 30)
...
```{r CD4 WNN}
cd4 <- FindMultiModalNeighbors(cd4, reduction.list = list("pca", "lsi"), dims.list = list(1:12, 1:12))
...
cd8 <- FindClusters(cd8, resolution = 0.4, graph.name = "wsnn")##0.5 is the default resolution, if you increase the number, you will get more clusters
DimPlot(cd8, group.by ="seurat_clusters", split.by = "dataset",cols = cd8_clus, reduction = "wnn.umap")
```

```{r Cluster CD4}
cd4 <- FindNeighbors(cd4, dims = 1:12 )#number of dimensions i used, no need to change
cd4 <- FindClusters(cd4, resolution = 0.45, graph.name = "wsnn")
DimPlot(cd4, group.by ="seurat_clusters", split.by = "dataset", cols = cd4_clus, reduction = "wnn.umap")
```
...
# saveRDS(t, file.path(CDS_DIR, "MO_filtered_101521.rds"))
# saveRDS(mo, file.path(CDS_DIR, "MO_unfiltered_101521.rds"))
```
...
DoHeatmap(cd8, features = top_n$gene, group.by = "dataset", slot = "scale.data", group.colors =  cd8_cols)+scale_fill_gradientn(colors = h_cols)
```
...
DefaultAssay(cd8)<- "SCT"
DotPlot(cd8, features = c("Tox","Pdcd1","Lag3","Havcr2","Eomes","Tbx21","Prf1", "Gzmb", "Bach2", "Il7r","Il18r1", "Lef1", "Cd44", "Cd226", "Mki67"), group.by = "seurat_clusters", cols = c("lightgrey", "red3"), col.min = -2.5, col.max = 2.5, dot.min = 0, dot.scale = 5, cluster.idents = F, scale.by = "size", scale.min = NA, scale.max = NA) +theme(axis.text.x=element_text(size=9), axis.text.y=element_text(size=9)) +xlab("Gene") + ylab("Group") + RotatedAxis()
```

```{r CD4 Cluster Dot Plot}
DefaultAssay(cd4)<- "SCT"
DotPlot(cd4, features = c("Foxp3","Tox","Pdcd1","Lag3","Havcr2","Eomes","Tbx21","Prf1", "Gzmb", "Bach2", "Il7r","Il18r1", "Lef1", "Cd44", "Cd226", "Mki67"), group.by = "seurat_clusters", cols = c("lightgrey", "red3"), col.min = -2.5, col.max = 2.5, dot.min = 0, dot.scale = 5, cluster.idents = F, scale.by = "size", scale.min = NA, scale.max = NA) +theme(axis.text.x=element_text(size=9), axis.text.y=element_text(size=9)) +xlab("Gene") + ylab("Group")
```
...
VlnPlot(stem, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
...
VlnPlot(cd8, features = "Stem_Like1", group.by = "dataset", pt.size = 0, cols = cd8_cols, log = F, y.max = max(cd8$Stem_Like1) +0.2, slot = "scale.data")+stat_summary(fun="mean", geom = "point")+stat_compare_means(comparisons = com, label = "p.format")
```

```{r Feature plot CD8 Stem SCT}
FeaturePlot(cd8, features = "Stem_Like1", max.cutoff = "q95", min.cutoff = "q5" ,cols = c("lightgray", "firebrick2"), reduction = "wnn.umap")
```
...
VlnPlot(cd8, features = "Stem_Like_Acc1", group.by = "dataset", pt.size = 0, cols = cd8_cols, log = F, y.max = max(cd8$Stem_Like_Acc1) +0.2, slot = "scale.data")+stat_summary(fun="mean", geom = "point")+stat_compare_means(comparisons = com, label = "p.format")
```

```{r Feature plot CD8 Stem Accessibility}
FeaturePlot(cd8, features = "Stem_Like_Acc1", max.cutoff = "q95", min.cutoff = "q5", cols = c("lightgray", "blue"), reduction = "wnn.umap")
```
...
VlnPlot(cd4, features = "Stem_Like1", group.by = "dataset", pt.size = 0, cols = cd4_cols, log = F, y.max = max(cd4$Stem_Like1) +0.2, slot = "scale.data")+stat_summary(fun="mean", geom = "point")+stat_compare_means(comparisons = com, label = "p.format")
```

```{r Feature plot CD4 Stem SCT}
FeaturePlot(cd4, features = "Stem_Like1", max.cutoff = "q95", min.cutoff = "q5" ,cols = c("lightgray", "firebrick2"), reduction = "wnn.umap")
```
...
VlnPlot(cd4, features = "Stem_Like_Acc1", group.by = "dataset", pt.size = 0, cols = cd4_cols, log = F, y.max = max(cd4$Stem_Like_Acc1) +0.2, slot = "scale.data")+stat_summary(fun="mean", geom = "point")+stat_compare_means(comparisons = com, label = "p.format")
```

```{r Feature plot CD8 Stem Accessibility }
FeaturePlot(cd4, features = "Stem_Like_Acc1", max.cutoff = "q95", min.cutoff = "q5", cols = c("lightgray", "blue"), reduction = "wnn.umap")
```
...
VlnPlot(cd8,features = "Tex1", pt.size=0, group.by = "dataset", cols = cd8_cols ,log = F, y.max = max(cd8$Tex1) +0.05, slot = "scale.data")+stat_summary(fun="mean", geom = "point")+stat_compare_means(comparisons = com, label = "p.format")
```

```{r Feature Plot CD8 Tex markers SCT}
FeaturePlot(cd8, features = "Tex1", max.cutoff = "q95", min.cutoff = "q5", cols = c("lightgray", "firebrick2"), reduction = "wnn.umap")
```
...
VlnPlot(cd8,features = "Tex_Acc1", pt.size=0, cols = cd8_cols, group.by = "dataset",log = F, y.max = max(cd8$Tex_Acc1) +0.05, slot = "scale.data")+stat_summary(fun="mean", geom = "point")+stat_compare_means(comparisons = com, label = "p.format")
```

```{r Feature Plot CD8 Tex markers Accessibility}
FeaturePlot(cd8, features = "Tex_Acc1",  max.cutoff = "q95", min.cutoff = "q10", reduction = "wnn.umap")+ggtitle(label= "T-Cell Exhaustion Gene Activity")
```
...
VlnPlot(cd4,features = "Tex1", pt.size=0, group.by = "dataset", cols = cd4_cols ,log = F, y.max = max(cd4$Tex1) +0.05, slot = "scale.data")+stat_summary(fun="mean", geom = "point")+stat_compare_means(comparisons = com, label = "p.format")
```

```{r Feature Plot cd4 Tex markers SCT}
FeaturePlot(cd4, features = "Tex1", max.cutoff = "q95", min.cutoff = "q5", cols = c("lightgray", "firebrick2"), reduction = "wnn.umap")
```
...
VlnPlot(cd4,features = "Tex_Acc1", pt.size=0, cols = cd4_cols, group.by = "dataset",log = F, y.max = max(cd4$Tex_Acc1) +0.05, slot = "scale.data")+stat_summary(fun="mean", geom = "point")+stat_compare_means(comparisons = com, label = "p.format")
```

```{r Feature Plot cd4 Tex markers Accessibility}
FeaturePlot(cd4, features = "Tex_Acc1",  max.cutoff = "q95", min.cutoff = "q5", reduction = "wnn.umap")+ggtitle(label= "T-Cell Exhaustion Gene Activity")
...
DoHeatmap(cd8, features = top_n$gene, group.colors =cd8_cols, group.by = "dataset", label=T)+scale_fill_gradientn(colors = h_cols)
```
